...
class TestReIDCore(unittest.TestCase):
    ...
    def test_find_match_vectorized(self, mock_core_class):
        ...
        with (
            ...
            engine._known_embeddings_matrix = np.stack([emb_a, emb_b]).astype(np.float32)
            engine._known_labels_list = ["A", "B"]
            engine._known_norms = np.array([1.0, 1.0], dtype=np.float32)
    ...
    def test_zero_norm_handling(self, mock_core_class):
        ...
        with (
            ...
            # Empty gallery
            engine._known_embeddings_matrix = np.empty((0, 256), dtype=np.float32)
            engine._known_labels_list = []
            engine._known_norms = np.empty((0,), dtype=np.float32)
            ...
            engine._known_embeddings_matrix = np.ones((1, 256), dtype=np.float32)
            engine._known_labels_list = ["A"]
            engine._known_norms = np.array([np.linalg.norm(np.ones(256))], dtype=np.float32)
